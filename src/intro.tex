...
\begin{tabular}{l|l}
Modo & Descrição\tabularnewline
\hline
Normal & Neste modo podemos deletar, copiar e formatar - atalho <Esc>\tabularnewline
Inserção & Digitação de texto - atalhos {}``i'',''a''.\tabularnewline
Visual & Seleção de blocos verticais e linhas inteiras - atalhos {}``v'',''V''\tabularnewline
Comando & Uma verdadeira linguagem de programação - atalho {}``:''\tabularnewline
\end{tabular}

Ao longo do estudo veremos detalhes de cada modo de operação!
