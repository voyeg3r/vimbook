...
ter vários significados, senão vejamos: Em modo normal pressionar duas vezes a
letra ``d'' \begin{verbatim} dd \end{verbatim} apaga a linha atual, já em modo
de inserção ele irá se comportar como se você estivesse usando qualquer outro
editor, ou seja, irá inserir duas vezes a letra ``d''.
...
\item [Normal] Neste modo podemos colar o que está no
 ``\verb|buffer|\footnote{No Vim a memória é chamada de {\em buffer}, assim como
...
ou tecla ``\verb|a|'' ({\em append}).  Para acessar este modo:  \verb+i,a,I,A,o,O+. Outra forma
de acessar este modo é usando a tecla \verb+<Insert>+, ao pressionar uma vez
entra no modo de inserção, se pressionar outra vez alterna para o modo replace, passando
a substituir os caracteres subsequêntes.
...
conteúdo no Vim.  Para acessar (a partir do modo normal):
