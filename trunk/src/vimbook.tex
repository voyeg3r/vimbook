...
%\linespread{1.05} % palatino necessita de valores diferentes
\usepackage{graphicx}
\usepackage{url}
...
%\url{run:caminhodoficheiro}{texto}
...
%  \begin{figure}[htb]
%    \center
%    \includegraphics{img/setas.png}
%    \label{setas}
%    \caption{Setas de direção}
%\end{figure}
% fim da imagem
...
\item Estando em {\em{modo de inserção}} pressionar ``\verb+j+'' na intenção de rolar o documento,
neste caso estaremos iserindo simplesmente a letra ``j''. 

\item Estando em {\em{modo normal}} acionar acidentalmente o ``\verb+<Caps Lock>+'' e tentar 
rolar o documento usando a letra ``\verb+J+'', o efeito é a junção das linhas, aliás 
um ótimo recurso quando a intenção é de fato esta.
...
o resultado é que o número que foi digitado inicalmente vira um quantificador par o que 
se digitou ao entrar no modo de inserção. A palavra aparecerá repedida na quantidade do 
número digitado. Assim, se você quizer digitar 10 vezes ``\verb+isto é um teste+'' faça assim
...
%O registro \verb|"*| é responsável por armazenar o último texto selecionado
%(p.e., através do mouse). O registro \verb|"+| é o denominado ``área de transferência'',
%normalmente utilizado para se transferir conteúdos entre aplicações---este
%registro é preenchido, por exemplo, usando-se a típica combinação {\tt Ctrl-v}
%encontrada em muitas aplicações. Já o registro \verb|"~| armazena o texto
%colado pela operação mais recente de ``arrastar-e-soltar'' ({\em
%drag-and-drop}). 
...
Lembrando que o anexando um simples ponto, a saída do comando torna-se o 
